...
def get_by_listing(listing_id):
    sql = """SELECT c.content, c.id comment_id, c.user_id, c.sent_date, c.edited_date, c.status comment_status, u.username, u.status user_status, u.image IS NOT NULL has_image, c.edited_date IS NOT NULL edited
...
def get_comment(comment_id):
    sql = """SELECT c.id, c.user_id, c.listing_id, c.content, c.status, c.sent_date, c.edited_date, u.username
             FROM comments c, users u
